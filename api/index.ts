...
app.get('/api/auth/me', authMiddleware, (req, res) => {
  const store = loadUserStore();
  const page = store.pages[req.user!.pageId];
  res.json({
    success: true,
    user: { id: req.user!.id, username: req.user!.username, pageId: req.user!.pageId },
    page: { id: req.user!.pageId, data: page },
  });
});
...
app.post('/api/upload', (req, res) => {
  ...
  try {
    ...
    const imageUrl = `http://localhost:${PORT}/uploads/${uniqueName}`;
...
io.on('connection', (socket) => {
  ...
  socket.on('message', ({ chatId, message }: { chatId: string; message: Omit<Message, 'id' | 'timestamp' | 'chatId'> }) => {
    ...
    // 通知页面所有者
    const pageId = chat.session.pageId;
    io.emit(`page-notification:${pageId}`, { chatId, message: fullMessage });
  });
